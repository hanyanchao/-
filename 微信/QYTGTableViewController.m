...
@interface QYTGTableViewController ()<UIScrollViewDelegate>
...
-(void)awakeFromNib
{
    ...
    self.tabBarController.tabBar.tintColor = [UIColor colorWithRed:0.3 green:0.8 blue:0.3 alpha:1];
...
-(void)addSubViewsForTableView
{
    ...
    CGFloat contentW = TableViewHeaderImageNum * QYScreenW;
...
{
    [refresh performSelector:@selector(endRefreshing) withObject:nil afterDelay:20];
}
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath {
    ...
    //把当前的数据模型从数据源（注意IndexPath.section）中删除
    [_tgModels removeObjectAtIndex:fromIndexPath.row];
    //把当前的数据模型插入到数据源（注意IndexPath.section）中
